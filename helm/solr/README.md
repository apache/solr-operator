...
```bash
helm install example apache-solr/solr --version 0.10.0-prerelease --set image.tag=9.9.0
```
...
```bash
helm upgrade example apache-solr/solr --version 0.10.0-prerelease --reuse-values --set image.tag=9.9.0
```
...
| image.tag | string | `"9.9.0"`         | The tag/version of Solr to run |
