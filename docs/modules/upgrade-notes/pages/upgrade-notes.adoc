...
If you want to skip versions when upgrading, be sure to check out the <<upgrading-minor-versions-v_x_,upgrading minor versions>> and <<upgrading-patch-versions-v__x,upgrading patch versions>> sections.
...
If you are upgrading from `v0.2.x` to `v0.3.x`, please follow the xref:development:upgrading-to-apache.adoc[Upgrading to Apache guide].
...
+
WARNING: This means that the default Zookeeper Storage type can change for users using ephemeral storage for Solr.
