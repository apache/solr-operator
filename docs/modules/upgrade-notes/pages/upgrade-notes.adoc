...
If you want to skip versions when upgrading, be sure to check out the <<upgrading-minor-versions-vxa-vxb,upgrading minor versions>> and <<upgrading-patch-versions-vxya-vzyb,upgrading patch versions>> sections.
...
| `v0.2.6` | YES | YES | YES | YES | NO
| `v0.2.7` | NO | YES | YES | YES | NO
| `v0.2.8` | NO | YES | YES | YES | NO
| `v0.3.x` | NO | YES | YES | YES | NO
| `v0.4.x` | NO | YES | YES | YES | NO
| `v0.5.x` | NO | NO | YES | YES | YES
| `v0.6.x` | NO | NO | YES | YES | YES
| `v0.7.x` | NO | NO | NO | YES | YES
| `v0.8.x` | NO | NO | NO | NO | YES
| `v0.9.x` | NO | NO | NO | NO | YES
|===
...
| `v0.2.7` | YES | YES | YES | NO | NO
| `v0.2.8` | YES | YES | YES | NO | NO
| `v0.3.x` | YES | YES | YES | NO | NO
| `v0.4.x` | YES | YES | YES | NO | NO
| `v0.5.x` | YES | YES | YES | NO | NO
| `v0.6.x` | YES | YES | YES | YES ^1^ | YES ^1^
| `v0.7.x` | YES | YES | YES | YES ^1^ | YES ^1^
| `v0.8.x` | NO | NO | YES | YES ^1^ | YES
| `v0.9.x` | NO | NO | YES | YES ^1^ | YES
|===

*Exceptions*

* ^1^ `SolrTLS` and `SolrClientTLS` are not supported
...
However, no matter the upgrade, always read the <<upgrade-warnings-and-notes,Upgrade Notes>> for every version between
...
Be sure to carefully read the <<upgrade-warnings-and-notes,Upgrade Warnings>> to be sure you are not using deprecated options.
...
Installing the Solr Operator, especially via the https://artifacthub.io/packages/helm/apache-solr/solr-operator[Helm Chart],
...
When upgrading the Solr Operator, you may need to upgrade the https://github.com/pravega/zookeeper-operator[Zookeeper Operator] at the same time.
...
----
# Just replace the Solr CRDs and all CRDs it might depend on (e.g. ZookeeperCluster)
kubectl replace -f "http://solr.apache.org/operator/downloads/crds/v{operator-version}/all-with-dependencies.yaml"
helm upgrade solr-operator apache-solr/solr-operator --version {operator-version}
----
...
* *The minimum supported Solr version is now 8.11*
  ...
  See the <<solr-versions,version compatibility matrix>> for more information.
...
* *Kubernetes support is now limited to 1.22+.*
  ...
  See the <<kubernetes-versions,version compatibility matrix>> for more information.
...
* The new `SolrCloud.spec.scaling.vacatePodsOnScaleDown` option is enabled by default.
  ...
  More information can be found in the xref:solr-cloud:scaling.adoc#solr-pod-scale-down[Solr Pod Scale-Down] documentation.
...
* The required version of the https://github.com/pravega/zookeeper-operator[Zookeeper Operator] to use with this version has been upgraded from `v0.2.14` to `v0.2.15`.
  ...
  Please refer to the https://github.com/pravega/zookeeper-operator/releases[Zookeeper Operator release notes] before upgrading.
  Make sure to install the correct version of the Zookeeper Operator CRDs, as <<upgrading-the-zookeeper-operator,shown above>>.

* `PodDisruptionBudgets` are now created by default alongside SolrCloud instances.
  The maximum number of pods allowed down at any given time is aligned with the xref:solr-cloud:solr-cloud-crd.adoc#update-strategy[Managed Update settings] provided in the spec.
...
* The Solr Operator now tries to limit connectivity to pods before they are deleted, for rolling updates or other reasons.
  ...
  Refer to the xref:solr-cloud:managed-updates.adoc#pod-readiness-during-updates[Managed Updates documentation].
...
* The required version of the https://github.com/pravega/zookeeper-operator[Zookeeper Operator] to use with this version has been upgraded from `v0.2.12` to `v0.2.14`.
  If you use the Solr Operator helm chart, then by default the new version of the Zookeeper Operator will be installed as well.
  Refer to the helm chart documentation if you want to manage the Zookeeper Operator installation yourself.
  Please refer to the https://github.com/pravega/zookeeper-operator/releases[Zookeeper Operator release notes] before upgrading.
  Make sure to install the correct version of the Zookeeper Operator CRDS, as <<upgrading-the-zookeeper-operator,shown above>>.
...
+
Users using the S3 persistence option should try to use the xref:solr-backup:backup-repositories.adoc#s3-backup-repositories[S3 backup repository] instead. This requires Solr 8.10 or higher.
...
* The required version of the https://github.com/pravega/zookeeper-operator[Zookeeper Operator] to use with this version has been upgraded from `v0.2.9` to `v0.2.12`.
  If you use the Solr Operator helm chart, then by default the new version of the Zookeeper Operator will be installed as well.
  Refer to the helm chart documentation if you want to manage the Zookeeper Operator installation yourself.
  Please refer to the https://github.com/pravega/zookeeper-operator/releases[Zookeeper Operator release notes] before upgrading.
  Make sure to install the correct version of the Zookeeper Operator CRDS, as <<upgrading-the-zookeeper-operator,shown above>>.
...
* The `SolrCollection` and `SolrCollectionAlias` have been removed. Please use the Solr APIs to manage these resources instead.
  Discussion around the removal can be found in https://github.com/apache/solr-operator/issues/204[Issue #204].

* The required version of the https://github.com/pravega/zookeeper-operator[Zookeeper Operator] to use with this version has been upgraded from `v0.2.6` to `v0.2.9`.
  If you use the Solr Operator helm chart, then by default the new version of the Zookeeper Operator will be installed as well.
  Refer to the helm chart documentation if you want to manage the Zookeeper Operator installation yourself.
  Please refer to the https://github.com/pravega/zookeeper-operator/releases[Zookeeper Operator release notes] before upgrading.
...
* The default supported version of the Zookeeper Operator has been upgraded to `v0.2.6`.
  ...
  You can find examples of the zookeeper operator as well as solrClouds that use provided zookeepers in the https://github.com/apache/solr-operator/blob/main/example[examples] directory.
  Please refer to the https://github.com/pravega/zookeeper-operator/releases[Zookeeper Operator release notes] before upgrading.
