...
. <<setup-kubernetes-and-dependencies,Setup Kubernetes and Dependencies>>
.. <<setup-docker-for-mac-with-k8s,Setup Docker for Mac with K8S>>
.. <<install-an-ingress-controller,Install an Ingress Controller to reach the cluster on localhost>>
. <<install-the-solr-operator,Install Solr Operator>>
. <<start-an-example-solr-cloud-cluster,Start your Solr cluster>>
. <<create-a-collection-and-index-some-documents,Create a collection and index some documents>>
. <<scale-from-3-to-5-nodes,Scale from 3 to 5 nodes>>
.. <<horizontal-pod-autoscaler-hpa,Using the Horizontal Pod Autoscaler>>
. <<upgrade-to-newer-version,Upgrade to newer Solr version>>
. <<install-kubernetes-dashboard-optional,Install Kubernetes Dashboard (optional)>>
. <<delete-the-solrcloud-cluster-named-example,Delete the solrCloud cluster named 'example'>>
