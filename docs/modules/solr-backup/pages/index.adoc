...
* <<supported-repository-types,Repository Types>>
** <<gcs-backup-repositories,GCS>>
** <<s3-backup-repositories,S3>>
** <<volume-backup-repositories,Volume>>
...
IMPORTANT: All persistent volumes used with Volume Repositories must have `accessMode: ReadWriteMany` set, otherwise the backups will not succeed.
