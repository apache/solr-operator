...
Test the release candidate in a Kind cluster
    ...
    -t  Full solr image, or image tag (for the official Solr image), to test with (e.g. apache/solr-nightly:9.0.0, 9.9.0). (Optional, defaults to a compatible version)
EOF
}
